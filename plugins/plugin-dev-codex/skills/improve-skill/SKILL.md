...
3. Apply the active `skill-creator` guidance from the current Codex or Claude environment. Do not depend on an absolute user-specific path.
...
- move bulky details into the target skill's own `references/` or `scripts/` directory
...
- `../plugin-eval/references/chat-first-workflows.md`
