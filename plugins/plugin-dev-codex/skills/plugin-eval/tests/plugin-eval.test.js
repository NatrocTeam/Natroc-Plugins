...
} from "../src/core/benchmark-events.js";
...
const cliPath = path.join(skillRoot, "scripts", "plugin-eval.js");
...
async function writeBlockScalarSkillFixture(
  rootPath,
  { style = ">", descriptionLines, bodyLines = 3 },
) {
  await fs.mkdir(rootPath, { recursive: true });
...
async function writePluginBudgetFixture(rootPath) {
  ...
  await fs.mkdir(path.join(rootPath, "skills", "router", "agents"), {
    recursive: true,
  });
  await fs.writeFile(
  ...
  await fs.mkdir(path.join(rootPath, "skills", "specialist", "agents"), {
    recursive: true,
  });
  await fs.writeFile(
...
async function createFakeCodexExecutable(rootPath) {
  ...
  const executablePath = path.join(binDir, isWindows ? "codex.cmd" : "codex");
...
:parse
  ...
  goto parse
:run
  for %%I in ("%final%") do if not exist "%%~dpI" mkdir "%%~dpI"
  > "%final%" echo Implemented benchmark fixture.
  > "%workspace%\\generated.ts" echo export const generated = 1;
  > "%workspace%\\generated.test.ts" echo import { generated } from "./generated";
  >> "%workspace%\\generated.test.ts" echo export default generated;
  echo {"type":"thread.started","thread_id":"thread-test"}
  echo {"type":"tool.called","tool_name":"functions.exec_command"}
  echo {"type":"shell.command","command":"npm test"}
  echo {"type":"turn.completed","usage":{"input_tokens":120,"output_tokens":45,"total_tokens":165}}
  exit /b 0
)
...
test("analyze minimal skill and render markdown/html", async () => {
  ...
  assert.ok(
    result.metrics.some((metric) => metric.id === "trigger_cost_tokens"),
  );
...
test("parses folded and literal YAML block scalars in frontmatter", async () => {
  ...
  assert.equal(
    folded.data.description,
    "Use when the task needs a folded block scalar.",
  );
...
test("explicit-only skill policy removes trigger budget", async () => {
  ...
  assert.equal(
    result.budgets.invocation_policy.allow_implicit_invocation,
    false,
  );
  ...
  assert.ok(
    !result.checks.some(
      (check) => check.id === "trigger_cost_tokens-budget-high",
    ),
  );
});
...
test("plugin budget excludes explicit-only specialist skills from implicit active budget", async () => {
  ...
  const result = await analyzePath(tempDir);
  const triggerLabels = result.budgets.trigger_cost_tokens.components.map(
    (component) => component.label,
  );
  const invokeLabels = result.budgets.invoke_cost_tokens.components.map(
    (component) => component.label,
  );
  const explicitLabels =
    result.budgets.explicit_only_invoke_cost_tokens.components.map(
      (component) => component.label,
    );
  ...
  assert.ok(
    result.metrics.some(
      (metric) => metric.id === "explicit_only_invoke_cost_tokens",
    ),
  );
});
...
test("ingests lcov, coverage.xml, and coverage-final.json artifacts", async () => {
  ...
  const result = await analyzePath(samplePath);
  const coveragePercent = result.metrics.find(
    (metric) => metric.id === "coverage_percent",
  )?.value;

  assert.equal(coveragePercent, 82);
  assert.equal(
    result.metrics.find((metric) => metric.id === "coverage_artifact_count")
      ?.value,
    3,
  );
});
...
test("merges custom metric pack output without changing the core summary", async () => {
  ...
  const result = await analyzePath(skillPath, {
    metricPackManifests: [
      path.join(fixturesRoot, "metric-pack", "manifest.json"),
    ],
  });
...
test("ingests observed usage files and compares estimates against real sessions", async () => {
  ...
  assert.equal(
    result.metrics.find((metric) => metric.id === "observed_usage_sample_count")
      ?.value,
    3,
  );
  assert.ok(
    result.metrics.some(
      (metric) => metric.id === "estimate_vs_observed_input_ratio",
    ),
  );
...
test("init-benchmark writes a beginner-friendly starter config", async () => {
  ...
  const config = JSON.parse(
    await fs.readFile(
      path.join(skillPath, ".plugin-eval", "benchmark.json"),
      "utf8",
    ),
  );
...
test("parses codex json event streams and extracts usage plus shell activity", async () => {
  const stream = [
    ...
    JSON.stringify({
      type: "turn.completed",
      usage: { input_tokens: 101, output_tokens: 44, total_tokens: 145 },
    }),
  ].join("\n");
...
test("formats command paths with home shorthand for user-facing commands", () => {
  const formatted = formatCommandPath(
    path.join(os.homedir(), ".codex", "skills", "game-dev"),
    {
      cwd: repoRoot,
    },
  );
...
test("workflow guide next action uses the first actionable local command", async () => {
  ...
  assert.equal(
    ...
    `plugin-eval init-benchmark ${formatCommandPath(skillPath)}`,
  );
});
...
test("benchmark run writes usage logs and generated-code analysis from a codex-cli run", async () => {
  ...
  const payload = await runBenchmark(skillPath, {
    ...
    processRunner: async ({ kind, args }) => {
      ...
      if (kind === "codex") {
        ...
        await fs.writeFile(
          path.join(workspacePath, "generated.ts"),
  ...
  assert.equal(
    payload.scenarios[0].generatedCode.metrics.find(
      (metric) => metric.id === "ts_file_count",
    )?.value,
    2,
  );
...
test("provisionBenchmarkWorkspace seeds auth and config into the isolated codex home", async () => {
  ...
  await fs.writeFile(
    path.join(codexHomeSource, "auth.json"),
    '{"token":"test"}\n',
    "utf8",
  );
  await fs.writeFile(
  ...
  try {
    assert.equal(
      await fs.readFile(
        path.join(provisioned.codexHomePath, "auth.json"),
        "utf8",
      ),
      '{"token":"test"}\n',
    );
    assert.equal(
      await fs.readFile(
        path.join(provisioned.codexHomePath, "config.toml"),
        "utf8",
      ),
      'model = "gpt-5.4"\n',
    );
...
test("CLI analyze, report, compare, and explain-budget commands work together", async () => {
  ...
  assert.match(
    await fs.readFile(comparePath, "utf8"),
  ...
  const { stdout } = await execFileAsync(
    nodeBin,
    [cliPath, "explain-budget", skillPath],
    {
      cwd: repoRoot,
    },
  );
  ...
  assert.match(
    await fs.readFile(measuresPath, "utf8"),
    /Recommended Next Step/,
  );
...
test("CLI init-benchmark and benchmark commands work together with a fake codex executable", async () => {
  ...
  assert.match(
    await fs.readFile(runPath, "utf8"),
    /Codex version: codex-cli fake-test/,
  );
});
...
test("workflow guide routes analysis requests into report plus benchmark setup", async () => {
  ...
  assert.equal(
    guide.startHere.firstCommand,
    `plugin-eval analyze ${formatCommandPath(skillPath)} --format markdown`,
  );
  assert.ok(
    guide.startHere.commands.some((command) =>
      command.includes("plugin-eval init-benchmark"),
    ),
  );
...
test("shipped plugin surfaces advertise beginner chat prompts", async () => {
  ...
  const umbrellaSkill = await fs.readFile(
    path.join(repoRoot, "skills", "plugin-eval", "SKILL.md"),
    "utf8",
  );
