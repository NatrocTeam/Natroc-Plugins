...
The core engine lives under `skills/plugin-eval/src/core/` and owns:
...
Evaluators live under `skills/plugin-eval/src/evaluators/` and return normalized fragments:
...
Renderers live under `skills/plugin-eval/src/renderers/` and consume the canonical evaluation payload.
...
Runtime source directories, tests, fixtures, and `.plugin-eval/` run artifacts are excluded from deferred prompt-budget scoring. They are still inspected by code, coverage, and benchmark evaluators where relevant, but they do not represent agent instruction context by themselves.
...
3. Run live in an isolated temporary workspace.
