...
export function compareResults(before, after) {
  ...
  const resolvedFailures = [...beforeFailures].filter(
    (id) => !afterFailures.has(id),
  );
  const newFailures = [...afterFailures].filter(
    (id) => !beforeFailures.has(id),
  );
  ...
  const payload = {
    ...
    budgetDelta: {
      trigger_cost_tokens:
        (after.budgets.trigger_cost_tokens?.value || 0) -
        (before.budgets.trigger_cost_tokens?.value || 0),
      invoke_cost_tokens:
        (after.budgets.invoke_cost_tokens?.value || 0) -
        (before.budgets.invoke_cost_tokens?.value || 0),
      deferred_cost_tokens:
        (after.budgets.deferred_cost_tokens?.value || 0) -
        (before.budgets.deferred_cost_tokens?.value || 0),
    },
    resolvedFailures,
    newFailures,
