...
function summarizeSamples(samples) {
  ...
  return {
    trigger_cost_tokens: [
      quantile(
        samples.map((sample) => sample.trigger_cost_tokens.value),
        0.5,
      ),
      quantile(
        samples.map((sample) => sample.trigger_cost_tokens.value),
    ...
    invoke_cost_tokens: [
      quantile(
        samples.map((sample) => sample.invoke_cost_tokens.value),
        0.5,
      ),
      quantile(
        samples.map((sample) => sample.invoke_cost_tokens.value),
    ...
    deferred_cost_tokens: [
      quantile(
        samples.map((sample) => sample.deferred_cost_tokens.value),
        0.5,
      ),
      quantile(
        samples.map((sample) => sample.deferred_cost_tokens.value),
...
export async function loadBudgetBaseline() {
  ...
  const curatedPluginRoot = path.join(
    home,
    ".codex",
    "plugins",
    "cache",
  ...
  const tempPluginRoot = path.join(
    home,
    ".codex",
    ".tmp",
    "plugins",
