...
function rankActionableChecks(checks) {
  return checks
    ...
    .sort((left, right) => {
      ...
      if (
        (CATEGORY_PRIORITY[left.category] ?? 99) !==
        (CATEGORY_PRIORITY[right.category] ?? 99)
      ) {
        return (
          (CATEGORY_PRIORITY[left.category] ?? 99) -
          (CATEGORY_PRIORITY[right.category] ?? 99)
...
function buildWhyBullets(result, summary, failedErrors, warningSignals) {
  ...
  if (failedErrors > 0) {
    bullets.push(
      `${failedErrors} failing error check${failedErrors === 1 ? "" : "s"} are driving the highest-confidence problems.`,
    );
  }

  if (warningSignals > 0) {
    bullets.push(
      `${warningSignals} warning signal${warningSignals === 1 ? "" : "s"} still need cleanup before this feels polished.`,
    );
  }
  ...
  if (summary.categoryDeductions.length > 0) {
    ...
    bullets.push(
      `${topCategory.category} is the largest source of score loss at -${topCategory.totalPenalty} points.`,
    );
  }

  if (
    ["heavy", "excessive"].includes(
      result.budgets?.trigger_cost_tokens?.band,
    ) ||
  ...
  if (result.observedUsage?.sampleCount) {
    bullets.push(
      `Observed usage is available from ${result.observedUsage.sampleCount} sample${result.observedUsage.sampleCount === 1 ? "" : "s"}.`,
    );
  } else {
    bullets.push(
  ...
  if (bullets.length === 0) {
    bullets.push(
      `Current grade is ${summary.grade} with ${summary.riskLevel} reported risk.`,
    );
  }
...
function buildRiskReasons({
  ...
  if (score < GRADE_THRESHOLDS.C) {
    reasons.push(
      `Overall score is below ${GRADE_THRESHOLDS.C}, which the evaluator treats as high risk.`,
    );
  ...
  if (checkSummary.info > 0 && reasons.length === 0) {
    reasons.push(
      "No failing or warning checks were found; remaining items are informational only.",
    );
  }
  if (reasons.length === 0) {
...
export function computeSummary(result) {
  ...
  const warningSignals = result.checks.filter(
    (check) => check.status === "warn",
  ).length;
  const riskLevel =
    failedErrors > 0 || score < 70
      ? "high"
      : warningSignals > 0 || score < 85
        ? "medium"
        : "low";
...
export function enrichSummary(result, summary) {
  const deductions = Array.isArray(summary?.deductions)
    ? summary.deductions
    : buildDeductions(result.checks);
  ...
  const actionableChecks = rankActionableChecks(result.checks).map(
    summarizeFinding,
  );
  const nextSummary = {
    ...summary,
  ...
  return {
    ...
    whyBullets:
      Array.isArray(nextSummary.whyBullets) && nextSummary.whyBullets.length > 0
        ...
        : buildWhyBullets(
            result,
            { ...nextSummary, categoryDeductions },
            failedErrors,
            warningSignals,
          ),
    fixFirst:
