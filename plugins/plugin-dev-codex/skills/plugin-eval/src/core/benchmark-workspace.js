...
function testFilePattern(filePath) {
  return /(^|\/)(tests?|__tests__)\/|(\.test|\.spec)\.|(^|\/)test_[^/]+\.py$/.test(
    filePath,
  );
}
...
export async function provisionBenchmarkWorkspace({
  target,
  config,
  scenarioId,
}) {
  ...
  if (!(await isDirectory(sourcePath))) {
    throw new Error(
      `Benchmark workspace.sourcePath must be a directory: ${sourcePath}`,
    );
  }

  const tempRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), `plugin-eval-${scenarioId}-`),
  );
  ...
  if (config.targetProvisioning.mode === "isolated-skill-home") {
    ...
    config.targetProvisioning.mode === "workspace-plugin-marketplace"
  ) {
    installedTargetPath = await provisionPluginInstall(target, workspacePath);
  } else {
    throw new Error(
      `Unsupported target provisioning mode: ${config.targetProvisioning.mode}`,
    );
  }
  ...
  for (const [filePath, afterEntry] of afterSnapshot.entries()) {
    ...
    if (!beforeEntry) {
      added.push({
        ...
        lineDelta:
          typeof afterEntry.lineCount === "number"
            ? afterEntry.lineCount
            : null,
      });
      continue;
    }
    ...
    if (beforeEntry.hash !== afterEntry.hash) {
      modified.push({
        ...
        lineCountAfter: afterEntry.lineCount,
        lineDelta:
          typeof beforeEntry.lineCount === "number" &&
          typeof afterEntry.lineCount === "number"
...
export function summarizeWorkspaceDiff(diff) {
  ...
  return {
    ...
    generatedTestFileCount: diff.added.filter((entry) =>
      testFilePattern(entry.path),
    ).length,
    changedTestFileCount: changedFiles.filter((entry) =>
      testFilePattern(entry.path),
    ).length,
    changedTextLineDelta: changedFiles.reduce(
      (sum, entry) => sum + (entry.lineDelta || 0),
      0,
    ),
    changedFiles,
