...
function inferGoalFromRequest(requestText) {
  ...
  const signals = [
    ...
    [
      "benchmark",
      requestSignal(
        ["benchmark", "dry-run", "dry run", "starter scenario", "scenario"],
...
function commandsForGoal(goal, target, status) {
  ...
  if (goal === "budget") {
    return [
      `plugin-eval explain-budget ${commandTargetPath} --format markdown`,
    ];
  }
  ...
  if (goal === "benchmark") {
    if (!status.hasBenchmarkConfig) {
      return [`plugin-eval init-benchmark ${commandTargetPath}`];
    }
...
export async function buildWorkflowGuide(targetPath, options = {}) {
  ...
  const recommendedWorkflow =
    entries.find((entry) => entry.id === recommendedGoal) || entries[0];
