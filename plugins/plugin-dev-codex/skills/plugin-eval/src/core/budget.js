...
const DEFERRED_BUDGET_EXCLUDED_DIRS = new Set([
  ".plugin-eval",
  "__fixtures__",
  "fixtures",
  "src",
...
async function gatherDeferredTextFiles(rootPath, excludeFiles = []) {
  ...
  return files.filter(
    (filePath) =>
      ...
      !hasExcludedBudgetDir(rootPath, filePath),
  );
}

function hasExcludedBudgetDir(rootPath, filePath) {
  const segments = relativePath(rootPath, filePath).split("/");
  return segments.some((segment) => DEFERRED_BUDGET_EXCLUDED_DIRS.has(segment));
}
...
export async function computeSkillBudget(skillRoot) {
  ...
  const triggerComponents = invocationPolicy.allowImplicitInvocation
    ? [
        createComponent(
          "skill-name",
          skillPath,
          estimateTokenCount(name),
          "Always-loaded skill identifier",
        ),
        createComponent(
          "skill-description",
          skillPath,
  ...
  const deferredComponents = await computeDeferredComponents(skillRoot, [
    skillPath,
  ]);

  return {
    kind: "skill",
    method: invocationPolicy.allowImplicitInvocation
      ? "estimated-static"
      : "estimated-static-policy-aware",
    invocation_policy: {
...
export async function computePluginBudget(pluginRoot, manifest) {
  ...
  const invokeComponents = [
    createComponent(
      "plugin-manifest",
      manifestPath,
      estimateTokenCount(manifestContent),
      "Manifest load cost",
    ),
  ];
...
export async function computeBudgetProfile(target) {
  ...
  if (target.kind === "plugin") {
    const manifest = await readJson(
      path.join(target.path, ".codex-plugin", "plugin.json"),
    );
...
export function applyBudgetBands(rawBudget, baseline) {
  const profile =
    baseline?.[rawBudget.kind] || baseline?.directory || baseline?.skill;
  ...
  return {
    ...
    ...(rawBudget.invocation_policy
      ? { invocation_policy: rawBudget.invocation_policy }
      : {}),
    ...
    total_tokens: {
      value: total,
      band: [trigger.band, invoke.band, deferred.band].includes("excessive")
        ? "excessive"
