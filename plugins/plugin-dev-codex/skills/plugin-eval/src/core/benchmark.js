...
function defaultUsageLogPath(target) {
  return path.join(
    benchmarkDirectoryForTarget(target),
...
function normalizeScenario(scenario, index) {
  return {
    ...
    successChecklist: Array.isArray(scenario.successChecklist)
      ? scenario.successChecklist
      : [],
  };
}
...
function normalizeConfig(config, target) {
  ...
  if (!config || config.kind !== "plugin-eval-benchmark") {
    throw new Error(
      "Invalid benchmark config. Expected a plugin-eval benchmark config file.",
    );
  }
  ...
  return [
    {
      ...
      purpose:
        "Run a representative task that should clearly justify using this skill inside the workspace.",
    ...
    },
    {
      ...
      purpose:
        "Measure whether the skill can improve an implementation cleanly instead of restarting from scratch.",
    ...
    },
    {
      ...
      purpose:
        "Check whether the skill avoids overreaching when the task is only a partial fit.",
  ...
  return [
    {
      ...
      purpose:
        "Check whether the plugin routes a representative request through the right capability and finishes the task in the workspace.",
    ...
    },
    {
      ...
      purpose:
        "Measure whether a second request stays coherent when the plugin needs a different angle.",
    ...
    },
    {
      ...
      purpose:
        "Check whether the plugin narrows scope when the request is a weak match.",
...
async function createStarterBenchmarkConfig(target, options = {}) {
  ...
  return {
    ...
    scenarios:
      target.kind === "plugin"
        ? buildPluginScenarios(target)
        : buildSkillScenarios(target),
  };
}
...
async function analyzeChangedWorkspaceCode(
  workspacePath,
  ...
  return {
    ...
    artifacts: [
      ...tsAnalysis.artifacts,
      ...pyAnalysis.artifacts,
      ...coverageAnalysis.artifacts,
    ],
  };
}
...
function summarizeRunScenarios(runScenarios) {
  ...
  const completedScenarios = runScenarios.filter(
    (scenario) => scenario.status === "completed",
  ).length;
  const failedScenarios = runScenarios.filter(
    (scenario) => scenario.status !== "completed",
  ).length;
  const input = usageSamples.reduce(
    (sum, scenario) => sum + (scenario.usage?.input_tokens || 0),
    0,
  );
  const output = usageSamples.reduce(
    (sum, scenario) => sum + (scenario.usage?.output_tokens || 0),
    0,
  );
  const total = usageSamples.reduce(
    (sum, scenario) => sum + (scenario.usage?.total_tokens || 0),
    0,
  );
  const generatedFileCount = runScenarios.reduce(
    (sum, scenario) =>
      sum + (scenario.workspaceSummary?.generatedFileCount || 0),
    0,
  );
  const generatedTestFileCount = runScenarios.reduce(
    (sum, scenario) =>
      sum + (scenario.workspaceSummary?.generatedTestFileCount || 0),
    0,
  );
  const toolCallCount = runScenarios.reduce(
    (sum, scenario) => sum + (scenario.telemetry?.toolCallCount || 0),
    0,
  );
  const shellCommandCount = runScenarios.reduce(
    (sum, scenario) => sum + (scenario.telemetry?.shellCommandCount || 0),
    0,
  );
  const failedShellCommands = runScenarios.reduce(
    (sum, scenario) => sum + (scenario.telemetry?.failedShellCommandCount || 0),
    0,
  );
  const verifierPassCount = runScenarios.reduce(
    (sum, scenario) =>
      sum +
      scenario.verifierResults.filter((result) => result.status === "passed")
        .length,
    0,
  );
  const verifierFailCount = runScenarios.reduce(
    (sum, scenario) =>
      sum +
      scenario.verifierResults.filter((result) => result.status === "failed")
        .length,
    0,
  );
  ...
  return {
    ...
    averageInputTokens:
      usageSamples.length > 0
        ? Number((input / usageSamples.length).toFixed(2))
        : 0,
    averageOutputTokens:
      usageSamples.length > 0
        ? Number((output / usageSamples.length).toFixed(2))
        : 0,
    averageTotalTokens:
      usageSamples.length > 0
        ? Number((total / usageSamples.length).toFixed(2))
        : 0,
    generatedFileCount,
...
export async function initializeBenchmark(targetPath, options = {}) {
  ...
  const outputPath = path.resolve(
    options.outputPath || defaultBenchmarkConfigPath(target),
  );
...
export async function runBenchmark(targetPath, options = {}) {
  const target = await resolveTarget(targetPath);
  const { config, configPath, source } = await loadBenchmarkConfig(
    target,
    options,
  );
  const scenarios = (config.scenarios || [])
    .map(normalizeScenario)
    .filter((scenario) => scenario.userInput);
  if (scenarios.length === 0) {
    throw new Error(
      "Benchmark config does not contain any runnable scenarios.",
    );
  }
  if (options.dryRun) {
    throw new Error(
      "CLI-only benchmarking no longer supports --dry-run. Edit the benchmark config, then run `plugin-eval benchmark` for a real Codex execution.",
    );
  }
  ...
  const codexExecutable =
    ...
    process.env.PLUGIN_EVAL_CODEX_EXECUTABLE ||
    "codex";
  const runId = createRunId();
  const runDirectory = path.join(
    benchmarkRunsDirectoryForTarget(target),
    runId,
  );
  ...
  try {
    ...
    codexVersion =
      (versionResult.stdoutText || versionResult.stderrText || "unknown")
        .trim()
        .split(/\r?\n/)
        .pop() || "unknown";
  } catch {
  ...
  for (let index = 0; index < scenarios.length; index += 1) {
    ...
    const args = buildCodexExecArgs({
      config,
      model:
        options.model || config.runner.model || defaultModelForTarget(target),
    ...
    const verifierResults = await runVerifierCommands({
      commands: Array.isArray(config.verifiers?.commands)
    ...
    const codexSucceeded =
      codexRun.code === 0 && telemetry.finalStatus !== "failed";
    const verifiersPassed = verifierResults.every(
      (result) => result.status === "passed",
    );
    const scenarioStatus =
      codexSucceeded && verifiersPassed ? "completed" : "failed";

    if (telemetry.usage) {
      usageLines.push(
        buildObservedUsageLine(target, scenario, telemetry.usage),
      );
    }
    ...
    runScenarios.push({
      ...
      prompt: scenario.userInput,
      finalMessagePath: (await pathExists(finalMessagePath))
        ? finalMessagePath
        : null,
  ...
  if (usageOutPath) {
    ...
    await writeText(
      path.join(runDirectory, "observed-usage.jsonl"),
      `${usageLines.join("\n")}\n`,
    );
  }

  const resultOutPath = path.resolve(
    options.resultOutPath || path.join(runDirectory, "benchmark-run.json"),
  );
  ...
  const payload = {
    ...
    config: {
      ...
      verifierCount: Array.isArray(config.verifiers?.commands)
        ? config.verifiers.commands.length
        : 0,
    },
    runDirectory,
