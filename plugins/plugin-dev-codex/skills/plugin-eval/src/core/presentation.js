  ...
  if (!result.observedUsage?.sampleCount && hasBudgetPressure(result)) {
    return {
      ...
      command: buildStartCommand(
        result.target,
        `Measure the real token usage of this ${labelName}.`,
      ),
...
export function createWorkflowGuideNextAction(guide) {
  ...
  return {
    ...
    why:
      startHere?.routingExplanation ||
      guide.recommendedWorkflow?.summary ||
...
export function createMeasurementPlanNextAction(plan) {
  const toolset =
    plan.toolsets.find((entry) =>
      plan.recommendedToolsets.includes(entry.id),
    ) || plan.toolsets[0];
  const targetKind = targetTypeLabel(plan.target);

  return {
    label: toolset
      ? `Start with ${toolset.label}`
...
export function createBudgetNextAction(payload) {
  ...
  const hasHeavyBudget =
    ["heavy", "excessive"].includes(
      payload.budgets?.trigger_cost_tokens?.band,
    ) ||
...
export function createBenchmarkRunNextAction(payload) {
  const commandTargetPath = formatCommandPath(payload.target.path);
  return {
    label: payload.usageLogPath
      ? "Fold the usage log back into analysis"
    ...
    chatPrompt: payload.usageLogPath
      ? `What should I run next?`
      : `Help me benchmark this ${targetLabel(payload.target)}.`,
  };
}
...
export function createComparisonNextAction(diff) {
  ...
  return {
    label:
      diff.newFailures.length > 0
        ? "Address the new failures first"
