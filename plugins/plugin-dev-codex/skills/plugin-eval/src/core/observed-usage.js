...
function extractUsagePayload(candidate) {
  ...
  if (candidate.type === "response.done" && candidate.response?.usage) {
    return {
      ...
      label:
        candidate.metadata?.scenario ||
        candidate.response?.metadata?.scenario ||
        null,
    };
  }
...
function normalizeSnapshot(candidate, sourcePath, index) {
  ...
  const totalTokens =
    toNumber(usage.total_tokens) ?? (inputTokens || 0) + (outputTokens || 0);
  const cachedTokens =
...
async function loadSnapshotsFromFile(filePath) {
  ...
  parsedItems.forEach((item) =>
    collectSnapshots(item, resolvedPath, snapshots),
  );
  return snapshots;
}
  ...
  if (summary.sampleCount < 3) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "Observed usage coverage is too small to trust as a stable benchmark yet.",
        evidence: [`Samples collected: ${summary.sampleCount}`],
        remediation: [
          "Capture at least 5 to 10 representative sessions before treating observed usage as a baseline.",
        ],
      }),
    );
  }
    ...
    if (comparison.band === "drift" || comparison.band === "wide-drift") {
      checks.push(
        createCheck({
          ...
          message:
            "Static budget estimates differ meaningfully from observed input token usage.",
          evidence: [
    ...
    if (summary.cachedTokens.average > 0) {
      checks.push(
        createCheck({
          ...
          status: "info",
          message:
            "Observed runs include cached tokens, so repeated sessions are cheaper than the cold-start estimate.",
          evidence: [`Average cached tokens: ${summary.cachedTokens.average}`],
          remediation: [
            "Track cold-start and warm-cache sessions separately if you need tighter budgeting.",
          ],
        }),
      );
    }
...
export async function analyzeObservedUsage(usagePaths = [], rawBudget, target) {
  ...
  if (snapshots.length === 0) {
    throw new Error(
      "Observed usage files were provided, but no usage payloads could be parsed.",
    );
  }

  const inputTokens = buildStat(
    snapshots.map((snapshot) => snapshot.inputTokens),
  );
  const outputTokens = buildStat(
    snapshots.map((snapshot) => snapshot.outputTokens),
  );
  const totalTokens = buildStat(
    snapshots.map((snapshot) => snapshot.totalTokens),
  );
  const cachedTokens = buildStat(
    snapshots.map((snapshot) => snapshot.cachedTokens),
  );
  const reasoningTokens = buildStat(
    snapshots.map((snapshot) => snapshot.reasoningTokens),
  );
  const estimatedActiveTokens =
    rawBudget.trigger_cost_tokens.value + rawBudget.invoke_cost_tokens.value;
  ...
  const deltaRatio =
    estimatedActiveTokens > 0
      ? round(Math.abs(deltaTokens) / estimatedActiveTokens)
      : 0;
    ...
    files: [
      ...new Set(
        snapshots.map((snapshot) =>
          relativePath(process.cwd(), snapshot.sourcePath),
        ),
      ),
    ],
    inputTokens,
  ...
  const metrics = [
    createMetric({
      ...
      unit: "samples",
      band:
        observedUsage.sampleCount >= 5
          ? "good"
          : observedUsage.sampleCount >= 3
            ? "moderate"
            : "heavy",
    }),
    createMetric({
  ...
  return {
    ...
    artifacts: [
      createArtifact({
        ...
        description:
          "Observed token telemetry aggregated from local usage log files.",
        data: observedUsage,
      }),
    ],
  };
