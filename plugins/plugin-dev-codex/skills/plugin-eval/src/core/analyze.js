...
function normalizeExtensionPayload(extension) {
  return {
    ...extension,
    checks: extension.checks.map((check) => ({
      ...check,
      source: `extension:${extension.name}`,
    })),
    metrics: extension.metrics.map((metric) => ({
      ...metric,
      source: `extension:${extension.name}`,
    })),
...
export async function analyzePath(targetPath, options = {}) {
  ...
  if (target.kind === "skill") {
    ...
    result.checks.push(
      createCheck({
        ...
        status: "info",
        message:
          "The target is not a skill or plugin root, so only generic code, coverage, and budget analysis will run.",
        evidence: [path.resolve(target.path)],
        remediation: [
          "Point the analyzer at a skill directory or plugin root for richer structural checks.",
        ],
      }),
    );
  }
  ...
  addBudgetFindings(
    result,
    applyBudgetBands(rawBudget, baseline),
    baseline.evidence,
  );

  const observedUsageFragment = await analyzeObservedUsage(
    options.observedUsagePaths || [],
    rawBudget,
    target,
  );
  ...
  const extensions = await runMetricPacks(
    target,
    options.metricPackManifests || [],
  );
  ...
  result.measurementPlan.nextAction = createMeasurementPlanNextAction(
