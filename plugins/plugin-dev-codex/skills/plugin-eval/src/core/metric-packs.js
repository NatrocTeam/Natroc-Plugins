...
function normalizeCommand(manifest) {
  if (Array.isArray(manifest.command)) {
    return manifest.command.map((part, index) =>
      index === 0 && part === "node" ? process.execPath : part,
    );
  }
...
export async function runMetricPacks(target, manifestPaths = []) {
  ...
  for (const manifestPath of manifestPaths) {
    ...
    if (!(await pathExists(resolvedManifestPath))) {
      throw new Error(
        `Metric pack manifest not found: ${resolvedManifestPath}`,
      );
    }
    ...
    if (command.length === 0) {
      throw new Error(
        `Metric pack command is missing: ${resolvedManifestPath}`,
      );
    }

    const [bin, ...args] = command;
    const execution = spawnSync(bin, [...args, target.path, target.kind], {
