...
function renderNextAction(nextAction) {
  ...
  return [
    `- ${nextAction.label}`,
    `- Why: ${nextAction.why}`,
    ...(nextAction.chatPrompt
      ? [`- Chat request: "${nextAction.chatPrompt}"`]
      : []),
    ...(nextAction.command
      ? [`- Local command: \`${nextAction.command}\``]
      : []),
  ];
}
...
function renderFindingList(findings, emptyLabel) {
  ...
  return findings.map((finding) => {
    const parts = [
      `- [${finding.status}/${finding.severity}] ${finding.message}`,
    ];
    if (finding.why) {
...
function renderMetrics(metrics) {
  ...
  return metrics
    .map(
      (metric) =>
        `- ${metric.id}: ${metric.value} ${metric.unit} (${metric.band})`,
    )
    .join("\n");
}
...
function renderEvaluation(payload) {
  ...
  return [
    ...
    section(
      "Why It Matters",
      result.summary.whyBullets.map((item) => `- ${item}`),
    ),
    "",
    section(
      "Fix First",
      renderFindingList(
        result.summary.fixFirst,
        ...
        detailsBlock(
          ...
          [
            ...
            ...(result.improvementBrief?.measurementGoals?.length > 0
              ? result.improvementBrief.measurementGoals.map(
                  (item) => `- Measure: ${item}`,
                )
              : []),
    ...
    section("Details", [
      ...
      detailsBlock(
        ...
        guide.entrypoints
          .map(
            (entry) =>
              `- ${entry.label}: "${entry.chatPrompt}" -> ${entry.firstCommand}`,
          )
          .join("\n"),
      ),
    ]),
...
function renderBudgetExplanation(payload) {
  ...
  const fixFirst = [
    "- Nothing to fix from this view alone; use the next step to validate or contextualize the estimate.",
  ];

  if (
    ["heavy", "excessive"].includes(budgets.trigger_cost_tokens.band) ||
    ["heavy", "excessive"].includes(budgets.invoke_cost_tokens.band)
  ) {
    fixFirst.unshift(
      "- The active budget is heavy enough that measured usage is worth collecting before tuning copy or structure.",
    );
  }
  ...
  return [
    ...
    "",
    section("At a Glance", [...renderBudgets(budgets).split("\n")]),
    "",
    ...
    section(
      ...
      renderNextAction(budgetPayload.nextAction),
    ),
    "",
    section("Details", [
      detailsBlock(
        "Trigger components",
        budgets.trigger_cost_tokens.components
          .map(
            (component) => `- ${component.label}: ${component.tokens} tokens`,
          )
          .join("\n"),
      ),
      detailsBlock(
        "Invoke components",
        budgets.invoke_cost_tokens.components
          .map(
            (component) => `- ${component.label}: ${component.tokens} tokens`,
          )
          .join("\n"),
      ...
      detailsBlock(
        ...
        budgets.deferred_cost_tokens.components.length > 0
          ? budgets.deferred_cost_tokens.components
              .map(
                (component) =>
                  `- ${component.label}: ${component.tokens} tokens`,
              )
              .join("\n")
          : "- None",
      ),
      budgets.explicit_only_invoke_cost_tokens
        ? detailsBlock(
            ...
            budgets.explicit_only_invoke_cost_tokens.components.length > 0
              ? budgets.explicit_only_invoke_cost_tokens.components
                  .map(
                    (component) =>
                      `- ${component.label}: ${component.tokens} tokens`,
                  )
      ...
      detailsBlock(
        "Use From Codex Chat",
        renderWorkflowGuide(budgetPayload.workflowGuide),
      ),
    ]),
  ].join("\n");
}
...
function renderMeasurementPlanPayload(payload) {
  ...
  const recommendedToolsets = plan.toolsets.filter((toolset) =>
    plan.recommendedToolsets.includes(toolset.id),
  );
  ...
  return [
    ...
    section("Why It Matters", [
      `- ${plan.summary}`,
      ...recommendedToolsets.map(
        (toolset) => `- ${toolset.label}: ${toolset.goal}`,
      ),
    ]),
    "",
    section(
      ...
      recommendedToolsets.length > 0
        ? recommendedToolsets.map(
            (toolset) => `- ${toolset.label}: ${toolset.why}`,
          )
    ...
    section("Details", [
      ...
      detailsBlock(
        "Use From Codex Chat",
        renderWorkflowGuide(plan.workflowGuide),
      ),
    ]),
  ].join("\n");
}
...
function renderBenchmarkTemplateInit(payload) {
  ...
  return [
    ...
    section(
      "Why It Matters",
      benchmarkPayload.notes.map((item) => `- ${item}`),
    ),
    "",
    section("Fix First", [
      "- Edit the benchmark config so the scenarios match your real workflow before you trust the run.",
    ]),
    "",
    section(
      "Recommended Next Step",
      renderNextAction(benchmarkPayload.nextAction),
    ),
    "",
    section("Details", [
      detailsBlock(
        ...
        benchmarkPayload.setupQuestions?.length > 0
          ? benchmarkPayload.setupQuestions
              .map((item) => `- ${item}`)
              .join("\n")
      ...
      detailsBlock(
        "Next steps",
        benchmarkPayload.nextSteps.map((item) => `- ${item}`).join("\n"),
      ),
      detailsBlock(
        "Use From Codex Chat",
        renderWorkflowGuide(benchmarkPayload.workflowGuide),
      ),
    ]),
  ].join("\n");
}
...
function renderBenchmarkScenario(scenario, mode) {
  ...
  lines.push(
    `- Failed shell commands: ${scenario.telemetry?.failedShellCommandCount || 0}`,
  );
  lines.push(
    `- Usage availability: ${scenario.usageAvailability || "unavailable"}`,
  );
  ...
  if (scenario.workspaceSummary) {
    ...
    lines.push(
      `- Modified files: ${scenario.workspaceSummary.modifiedFileCount}`,
    );
    lines.push(
      `- Deleted files: ${scenario.workspaceSummary.deletedFileCount}`,
    );
    lines.push(
      `- Generated tests: ${scenario.workspaceSummary.generatedTestFileCount}`,
    );
  }
  ...
  if (scenario.verifierResults?.length > 0) {
    lines.push(
      "",
      ...scenario.verifierResults.map(
        (result) => `- Verifier [${result.status}]: ${result.command}`,
      ),
    );
  }
  ...
  if (scenario.generatedCode?.metrics?.length > 0) {
    lines.push(
      "",
      ...scenario.generatedCode.metrics.map(
        (metric) =>
          `- ${metric.id}: ${metric.value} ${metric.unit} (${metric.band})`,
      ),
    );
  ...
  if (scenario.generatedCode?.checks?.length > 0) {
    lines.push(
      "",
      ...scenario.generatedCode.checks.map(
        (check) => `- [${check.status}] ${check.message}`,
      ),
    );
  }
...
function renderBenchmarkRun(payload) {
  ...
  return [
    ...
    section("At a Glance", [
      ...
      ...(benchmarkPayload.usageLogPath
        ? [`- Usage log: \`${benchmarkPayload.usageLogPath}\``]
        : []),
    ...
    section("Details", [
      detailsBlock(
        ...
        benchmarkPayload.scenarios
          .map((scenario) =>
            renderBenchmarkScenario(scenario, benchmarkPayload.mode),
          )
          .join("\n\n"),
      ),
      detailsBlock(
...
function renderComparison(payload) {
  ...
  if (diff.newFailures.length > 0) {
    whyBullets.push(
      `- ${diff.newFailures.length} new failure${diff.newFailures.length === 1 ? "" : "s"} were introduced.`,
    );
  }
  if (diff.resolvedFailures.length > 0) {
    whyBullets.push(
      `- ${diff.resolvedFailures.length} failure${diff.resolvedFailures.length === 1 ? "" : "s"} were resolved.`,
    );
  }
