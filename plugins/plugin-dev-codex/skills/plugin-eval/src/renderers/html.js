...
function renderOptionalList(items, emptyLabel = "None.") {
  return renderList(
    Array.isArray(items) && items.length > 0 ? items : [emptyLabel],
  );
}
...
function renderObservedUsage(observedUsage) {
  ...
  if (observedUsage.estimateComparison) {
    items.push(
      `Estimated active tokens: ${observedUsage.estimateComparison.estimatedActiveTokens}`,
    );
    items.push(
      `Estimate delta: ${observedUsage.estimateComparison.deltaTokens}`,
    );
    items.push(
      `Estimate ratio: ${observedUsage.estimateComparison.deltaRatio} (${observedUsage.estimateComparison.band})`,
    );
  }
  ...
  return [
    ...
    renderList(
      guide.entrypoints.map(
        (entry) =>
          `${entry.label}: say "${entry.chatPrompt}" then run ${entry.firstCommand}`,
      ),
    ),
  ].join("");
}
