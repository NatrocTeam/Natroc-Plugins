...
function detectFunctions(lines) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const match =
      /^(\s*)(?:async\s+def|def)\s+([A-Za-z0-9_]+)\s*\(([^)]*)\)\s*(?:->\s*[^:]+)?\s*:/.exec(
        line,
      );
    if (!match) {
      continue;
  ...
  if (maxFunctionLength >= 80) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "At least one Python function is long enough to hurt readability.",
  ...
  if (totalLongLines > 0) {
    checks.push(
      createCheck({
        ...
        remediation: [
          "Wrap long expressions and keep docstrings or SQL fragments easier to scan.",
        ],
      }),
    );
  }
