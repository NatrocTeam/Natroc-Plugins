...
export async function evaluatePlugin(pluginRoot) {
  ...
  if (manifest.name && !isHyphenCase(manifest.name)) {
    checks.push(
      createCheck({
        ...
        remediation: [
          "Rename the plugin using lowercase letters, digits, and single hyphens only.",
        ],
        targetPath,
      }),
    );
  ...
  if (manifest.name && manifest.name !== path.basename(pluginRoot)) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "The plugin manifest name does not match the plugin directory name.",
        evidence: [
          `Directory: ${path.basename(pluginRoot)}`,
          `Manifest: ${manifest.name}`,
        ],
        remediation: [
          "Keep the plugin directory name and plugin.json name aligned.",
        ],
        targetPath,
      }),
    );
    ...
    if (typeof fieldValue !== "string" || !fieldValue.startsWith("./")) {
      checks.push(
        createCheck({
          ...
          remediation: [
            `Rewrite ${label} as a plugin-relative path like ./skills/.`,
          ],
          targetPath,
        }),
      );
    ...
    if (iface.defaultPrompt.length > 3) {
      checks.push(
        createCheck({
          ...
          remediation: [
            "Trim interface.defaultPrompt to three strong starters.",
          ],
          targetPath,
        }),
      );
    }
    const oversizedPrompts = iface.defaultPrompt.filter(
      (prompt) => prompt.length > 128,
    );
    if (oversizedPrompts.length > 0) {
      checks.push(
        createCheck({
          ...
          status: "warn",
          message:
            "One or more default prompts exceed the UI-friendly length budget.",
          evidence: oversizedPrompts.map(
            (prompt) => `${prompt.slice(0, 140)} (${prompt.length} chars)`,
          ),
          remediation: [
            "Keep default prompts under 128 characters and ideally closer to 50.",
          ],
          targetPath,
        }),
      );
  ...
  if (skillDirs.length === 0) {
    checks.push(
      createCheck({
        ...
        remediation: [
          "Add at least one skill under the configured skills path.",
        ],
        targetPath,
      }),
    );
  ...
  metrics.push(
    ...
    createMetric({
      ...
      category: "manifest",
      value: Array.isArray(iface.defaultPrompt)
        ? iface.defaultPrompt.length
        : 0,
      unit: "prompts",
      band:
