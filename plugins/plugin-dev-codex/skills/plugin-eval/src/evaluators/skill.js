...
export async function evaluateSkill(skillRoot, options = {}) {
  ...
  const supportFiles = (await walkFiles(skillRoot)).filter(
    (filePath) => filePath !== skillPath,
  );
  ...
  if (unexpectedKeys.length > 0) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "The skill frontmatter contains keys outside the common Codex skill conventions.",
        evidence: unexpectedKeys.map((key) => `Unexpected key: ${key}`),
        remediation: [
          "Remove non-standard frontmatter keys or move the metadata into references.",
        ],
        targetPath,
      }),
    );
  ...
  if (!frontmatter.name) {
    ...
    checks.push(
      createCheck({
        ...
        remediation: [
          "Rename the skill using lowercase letters, digits, and single hyphens only.",
        ],
        targetPath,
      }),
    );
    ...
    if (frontmatter.description.length > 1024) {
      checks.push(
        createCheck({
          ...
          status: "warn",
          message:
            "The skill description is long enough to create unnecessary always-loaded context cost.",
          evidence: [
            `Description length: ${frontmatter.description.length} characters`,
          ],
          remediation: [
            "Shorten the description and push details into SKILL.md or references.",
          ],
          targetPath,
        }),
      );
    ...
    if (!/use when/i.test(frontmatter.description)) {
      checks.push(
        createCheck({
          ...
          status: "warn",
          message:
            "The description does not clearly advertise when the skill should trigger.",
          evidence: [
            "Descriptions are the primary auto-load surface in Codex.",
          ],
          remediation: [
            "Rewrite the description to include a clear 'Use when ...' trigger sentence.",
          ],
          targetPath,
        }),
      );
  ...
  if (lines.length > 800) {
    checks.push(
      createCheck({
        ...
        status: "fail",
        message:
          "SKILL.md is extremely large for an always-loaded invocation surface.",
        evidence: [`Line count: ${lines.length}`],
        remediation: [
          "Move large details into references/ and keep SKILL.md focused on the core workflow.",
        ],
        targetPath,
      }),
    );
    ...
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "SKILL.md exceeds the recommended compact size for progressive disclosure.",
        evidence: [`Line count: ${lines.length}`],
        remediation: [
          "Trim repetitive detail and move long variants into references/.",
        ],
        targetPath,
      }),
    );
  ...
  if (
    lines.length > 350 &&
    supportFiles.filter((filePath) => filePath.includes("/references/"))
      .length === 0
  ) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "The skill is getting large without using references for progressive disclosure.",
        evidence: [
          "Large skills are easier to maintain when variants live under references/.",
        ],
        remediation: [
          "Move deep detail or edge-case variants into references/ and link them from SKILL.md.",
        ],
        targetPath,
      }),
    );
  ...
  if (brokenLinks.length > 0) {
    checks.push(
      createCheck({
        ...
        status: "fail",
        message:
          "The skill contains relative links that do not resolve inside the skill directory.",
  ...
  const extraDocs = supportFiles
    ...
    .filter((name) =>
      ["README.md", "CHANGELOG.md", "CONTRIBUTING.md"].includes(name),
    );
  if (extraDocs.length > 0) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "The skill includes extra documentation files that usually belong in references/ or outside the skill bundle.",
        evidence: extraDocs,
        remediation: [
          "Move reusable guidance into references/ and avoid extra top-level docs inside the skill bundle.",
        ],
        targetPath,
      }),
    );
  ...
  metrics.push(
    createMetric({
      ...
      unit: "lines",
      band:
        lines.length > 500 ? "heavy" : lines.length > 350 ? "moderate" : "good",
      targetPath,
    }),
    createMetric({
