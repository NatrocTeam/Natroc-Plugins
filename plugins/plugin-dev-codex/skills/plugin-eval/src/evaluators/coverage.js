...
async function parseCoverageXml(filePath) {
  ...
  if (coveredMatch && validMatch && Number(validMatch[1]) > 0) {
    return Number(
      ((Number(coveredMatch[1]) / Number(validMatch[1])) * 100).toFixed(2),
    );
  }
  return null;
}
...
async function parseCoverageJson(filePath) {
  ...
  const fileEntries = Object.values(payload).filter(
    (value) => value && typeof value === "object" && value.s,
  );
...
export async function analyzeCoverageArtifacts(rootPath) {
  ...
  if (fileSummaries.length > 0) {
    const bestCoverage = Math.max(
      ...fileSummaries.map((item) => item.coverage),
    );
    metrics.push(
      createMetric({
        id: "coverage_percent",
    ...
    if (bestCoverage < 70) {
      checks.push(
        createCheck({
          ...
          status: "warn",
          message:
            "Coverage artifacts were found, but the measured coverage is low.",
          evidence: fileSummaries.map(
            (item) => `${item.path}: ${item.coverage}%`,
          ),
          remediation: [
            "Increase test coverage on the code that carries the most complexity or risk.",
          ],
        }),
      );
    }
