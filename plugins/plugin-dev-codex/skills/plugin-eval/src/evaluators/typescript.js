...
import { readText, relativePath, toPosixPath } from "../lib/files.js";
...
function detectFunctions(lines) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    const match = startMatchers
      .map((matcher) => matcher.exec(line))
      .find(Boolean);
    if (!match) {
      continue;
    }
...
export async function analyzeTypeScriptFiles(filePaths, rootPath) {
  ...
  for (const filePath of filePaths) {
    ...
    const fileMaxComplexity =
      functions.length > 0
        ? functions.reduce((max, fn) => Math.max(max, fn.complexity), 0)
        : complexity;
    const fileMaxFunctionLength = functions.reduce(
      (max, fn) => Math.max(max, fn.length),
      0,
    );
    const fileMaxNesting = functions.reduce(
      (max, fn) => Math.max(max, fn.nesting),
      0,
    );
    ...
    if (
      /(^|\/)(tests?|__tests__)\/|(\.test|\.spec)\./.test(normalizedFilePath)
    ) {
      testFileCount += 1;
    }
  ...
  const averageFunctionLength =
    functionCount > 0
      ? Number((totalFunctionLength / functionCount).toFixed(2))
      : 0;
  const commentRatio =
    totalCodeLines > 0
      ? Number((totalCommentLines / totalCodeLines).toFixed(3))
      : 0;
  ...
  if (maxComplexity >= 18) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "At least one TypeScript function has high cyclomatic complexity.",
        evidence: [`Max complexity: ${maxComplexity}`],
        remediation: [
          "Split complex functions into smaller branches with clearer responsibilities.",
        ],
      }),
    );
  }
  ...
  if (maxFunctionLength >= 80) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "At least one TypeScript function is long enough to hurt readability.",
        evidence: [`Max function length: ${maxFunctionLength} lines`],
        remediation: [
          "Break large functions into smaller helpers with single-purpose logic.",
        ],
      }),
    );
  }
  ...
  if (totalLongLines > 0) {
    checks.push(
      createCheck({
        ...
        remediation: [
          "Wrap long expressions and object literals to keep scanability high.",
        ],
      }),
    );
  }
  ...
  if (filePaths.length > 0 && testFileCount === 0) {
    checks.push(
      createCheck({
        ...
        status: "warn",
        message:
          "TypeScript source files were found without matching test files.",
        evidence: [`Source files: ${filePaths.length}`],
        remediation: [
          "Add `.test.ts` or `.spec.ts` coverage for the main TypeScript logic.",
        ],
      }),
    );
  }
  ...
  artifacts.push(
    createArtifact({
      ...
      description:
        "Heuristic TypeScript quality and complexity breakdown by file.",
      data: {
        files: perFile,
      },
    }),
