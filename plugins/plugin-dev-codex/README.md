...
`plugin-dev-codex` is a Codex and Claude Code plugin bundle that ships the
`plugin-eval` local Node.js CLI.

It helps engineers evaluate a local skill or plugin, understand why it scored
that way, see what to fix first, explain token budgets, measure real usage, and
decide what to do next without having to memorize a command sequence first.

The installable plugin name is `plugin-dev-codex`. The bundled CLI command and
primary chat skill are named `plugin-eval`.

## What This Plugin Contains

- `skills/plugin-eval/scripts/plugin-eval.js`: the CLI entrypoint exposed as `plugin-eval`
- `skills/plugin-eval/src/`: the local evaluator engine used by the CLI
- `skills/plugin-eval/references/`: local technical docs for result schema,
  observed usage, metric packs, and chat-first workflows
- `.codex-plugin/plugin.json`: the Codex plugin manifest
- `.claude-plugin/plugin.json`: the Claude Code plugin manifest
- `agents/`: Claude Code root agents for plugin, skill, improvement, and metric
  pack workflows
- `skills/`: chat-facing skills for evaluation, improvement, metric packs, and
  routed start flows

Agent-facing support files live under `skills/plugin-eval/` so Claude Code can
load them through the declared `skills` manifest entry instead of depending on
arbitrary root plugin folders. Root files are limited to manifests, agents,
assets, package metadata, and README documentation.
...
From the plugin root (`plugins/plugin-dev-codex` in this monorepo):

```bash
node ./skills/plugin-eval/scripts/plugin-eval.js --help
```
...
```bash
node ./skills/plugin-eval/scripts/plugin-eval.js analyze ./skills/plugin-eval --format markdown
node ./skills/plugin-eval/scripts/plugin-eval.js analyze . --format markdown
```
...
If you prefer not to create a global link, keep using `node ./skills/plugin-eval/scripts/plugin-eval.js ...` directly.
...
## How It Works As An Agent Plugin

This directory is both a Codex and Claude Code plugin bundle. The Codex
manifest lives at [`.codex-plugin/plugin.json`](./.codex-plugin/plugin.json),
the Claude Code manifest lives at
[`.claude-plugin/plugin.json`](./.claude-plugin/plugin.json), and both expose
the skills under [`skills/`](./skills).
...
- Install Plugin
  ...
  ```
  claude plugin install plugin-dev-codex@natroc-plugins
  ```
...
- Install Plugin
  ```
  codex plugin add plugin-dev-codex@natroc-plugins
  ```
...
- in `~/.agents/plugins/marketplace.json`, `./plugins/plugin-dev-codex` resolves
  to `~/plugins/plugin-dev-codex`
- in `<workspace>/.agents/plugins/marketplace.json`,
  `./plugins/plugin-dev-codex` resolves to
  `<workspace>/plugins/plugin-dev-codex`
...
ln -sfn /absolute/path/to/plugins/plugin-dev-codex ~/plugins/plugin-dev-codex
```
...
```json
{
  ...
  "plugins": [
    {
      "name": "plugin-dev-codex",
      "source": {
        "source": "local",
        "path": "./plugins/plugin-dev-codex"
      },
      "policy": {
...
If you already have a marketplace file, append the `plugin-dev-codex` entry
...
ln -sfn /absolute/path/to/plugins/plugin-dev-codex /path/to/workspace/plugins/plugin-dev-codex
```
...
This repository already includes a workspace marketplace entry for
`plugin-dev-codex` in
[`../../.agents/plugins/marketplace.json`](../../.agents/plugins/marketplace.json).
If you reuse that file elsewhere, make sure `./plugins/plugin-dev-codex` exists
relative to the workspace root, or update the `source.path` accordingly.
...
- [Chat-first workflows](./skills/plugin-eval/references/chat-first-workflows.md)
- [Observed usage inputs](./skills/plugin-eval/references/observed-usage.md)
- [Technical design](./skills/plugin-eval/references/technical-design.md)
