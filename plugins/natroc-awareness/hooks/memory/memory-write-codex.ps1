...
$memoryDir = Join-Path $HOME ".codex" "memory"
...
if ($input -match '"session_id"\s*:\s*"([^"]*)"') {
    $sessionId = $matches[1]
} elseif ($input -match '"session"\s*:\s*"([^"]*)"') {
    $sessionId = $matches[1]
}
...
# Build the flag-write step. When session id is unavailable (older runtime
