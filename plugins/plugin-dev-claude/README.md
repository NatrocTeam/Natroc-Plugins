...
A comprehensive toolkit for developing Claude Code plugins with expert guidance
on hooks, MCP integration, plugin structure, settings, commands, agents, skills,
and marketplace-ready organization.

This plugin is packaged for Claude Code and Codex. Claude Code uses the root
agents, slash command, and skills; Codex uses the bundled skills and
Codex-compatible skill metadata for Claude plugin development work.
...
- Install Plugin
  ...
  ```
  claude plugin install plugin-dev-claude@natroc-plugins
  ```
...
- Install Plugin
  ```
  codex plugin add plugin-dev-claude@natroc-plugins
  ```
...
This plugin is part of `NatrocTeam/Natroc-Plugins`. To contribute improvements:

1. Fork `NatrocTeam/Natroc-Plugins`.
2. Make changes under `plugins/plugin-dev-claude/`.
3. Validate the affected plugin manifests, skills, agents, commands, and links.
4. Create a pull request with a focused summary and verification notes.
