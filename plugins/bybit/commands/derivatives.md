---
description: Bybit derivatives & conditional order trading
---

# Module: Derivatives Trading

> Use the `bybit` MCP server to fulfill the user's request using the endpoints and rules below.
...
>
> Example warning text (always include these keywords): "⚠️ **Large Order Warning**: This order has an estimated notional value of ~XX USD and requires ~YY USD in margin. Please confirm that your account **balance** is sufficient; if your **balance is insufficient**, the order will be rejected. Consider **reducing** the quantity before proceeding. This operation carries extremely **high risk**."
...
> trailingStop="500" means the stop triggers when price retraces by 500 USD. activePrice is the activation price (tracking begins only after this price is reached).
